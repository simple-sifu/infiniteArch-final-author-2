...
export class BooksRepository {
  ...
  load = async () => {
    console.log('BooksRepository.load() userModel=', this.userModel)
    const dto = await this.dataGateway.get(`/books?emailOwnerId=${this.userModel.email}`)
    console.log('BooksRepository.load() dto=', dto)
    if (dto?.success) {
      this.messagePm = {
        success: dto.success,
    ...
    console.log('BooksRepository.load() messagePm=', this.messagePm)
  ...
  reset = () => {
    console.log('BooksRepository.reset()')
